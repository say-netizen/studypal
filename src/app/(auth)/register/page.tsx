...
export default function RegisterPage() {
  ...
  useEffect(() => {
    getRedirectResult(auth()).then(async (result) => {
      if (!result) return;
      const pendingRole = (localStorage.getItem("pending_role") as Role | null) ?? "student";
      const pendingGrade = localStorage.getItem("pending_grade") ?? "";
      const { uid, displayName, email: userEmail } = result.user;
      await createUserDoc(uid, displayName ?? "ゲスト", userEmail ?? "", pendingRole, pendingGrade || undefined);
      localStorage.removeItem("pending_role");
      localStorage.removeItem("pending_grade");
      router.push(pendingRole === "parent" ? "/parent" : "/dashboard");
    }).catch(() => {});
  ...
  async function createUserDoc(
    ...
    await setDoc(doc(db(), "users", uid), {
      ...
      email: userEmail,
      role: userRole,
      grade: userRole === "student" ? (gradeOverride ?? grade ?? null) : null,
      plan: "free",
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (role === "student") localStorage.setItem("pending_grade", grade);
    ...
    <div className="w-full max-w-md">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 mb-3">
          ...
          <span className="text-2xl font-display font-black" style={{ color: "var(--color-brand-green)" }}>
            StudyPal
          </span>
        </div>
      ...
      {step === "role" ? (
        ...
        >
          <p className="text-center font-bold text-lg" style={{ color: "var(--color-text-primary)" }}>
            あなたはどちらですか？
          </p>
          ...
          <div className="flex flex-col gap-3">
            {(
              [
                { value: "student", emoji: "🎓", label: "生徒として登録", sub: "小5〜中3の学習者向け" },
                { value: "parent", emoji: "👨‍👩‍👧", label: "保護者として登録", sub: "お子さんの学習を見守る" },
              ] as const
              ...
              <button
                ...
                className="w-full rounded-2xl p-5 text-left transition-all duration-150 hover:scale-[1.01]"
                style={{
                  background: role === value ? "rgba(28,176,246,0.08)" : "var(--color-bg-secondary)",
                  border: role === value ? "2px solid var(--color-brand-blue)" : "2px solid var(--color-bg-tertiary)",
                }}
              >
          ...
          <button
            ...
            className="w-full rounded-full py-3 px-6 font-bold text-white transition-all duration-150 hover:-translate-y-0.5 active:translate-y-px"
            style={{ background: "var(--color-brand-blue)", boxShadow: "var(--shadow-btn)" }}
          >
            次へ →
          </button>
          ...
          <p className="text-center text-sm" style={{ color: "var(--color-text-secondary)" }}>
            すでにアカウントをお持ちの方は{" "}
            <Link href="/login" className="font-semibold hover:underline" style={{ color: "var(--color-brand-blue)" }}>
              ログイン
            </Link>
          </p>
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            <span
              ...
              style={{
                background: role === "parent" ? "rgba(155,93,229,0.12)" : "rgba(28,176,246,0.12)",
                color: role === "parent" ? "var(--color-brand-purple)" : "var(--color-brand-blue)",
              }}
            >
              {role === "parent" ? "👨‍👩‍👧 保護者" : "🎓 生徒"}
            </span>
          ...
          <div className="flex items-center gap-3 my-5">
            ...
            <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>またはメールで</span>
            <div className="flex-1 h-px" style={{ background: "var(--color-bg-tertiary)" }} />
          </div>
          ...
          <form onSubmit={handleRegister} className="flex flex-col gap-4">
            ...
            <button
              type="submit"
              disabled={loading || !name || !email || !password || (role === "student" && !grade)}
